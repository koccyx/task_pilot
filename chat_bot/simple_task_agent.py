...
DEFAULT_REVIEW_COLUMN_NAME = "На рассмотрении"
...
- Запросы "что на мне", "мои задачи", "покажи мои карточки" и аналогичные означают:
  вызови manage_cards с action="list", фильтром текущего пользователя,
  board="{DEFAULT_TASK_BOARD_NAME}" и column="{DEFAULT_REVIEW_COLUMN_NAME}".
- Когда пользователь просит список задач, которые ему нужно сделать, показывай только
  карточки из колонки "{DEFAULT_REVIEW_COLUMN_NAME}".
...
- Для фильтра по исполнителю используй owner_id, если Kaiten ID известен, иначе owner_name
  с именем пользователя в Kaiten. Для просмотра задач другого человека доска не обязательна,
  если пользователь не указал конкретную доску или колонку.
